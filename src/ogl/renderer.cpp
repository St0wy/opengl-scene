...
export
{
	...
	void stw::Renderer::InitPipelines()
	{
		...
		m_IrradiancePipeline.InitFromPath(
			"shaders/pbr/equirectangular.vert", "shaders/pbr/irradiance_convolution.frag");
	...
	void stw::Renderer::RenderShadowMaps(const std::array<glm::mat4, ShadowMapNumCascades>& lightViewProjMatrices)
	{
		...
		for (usize i = 0; i < lightViewProjMatrices.size(); i++)
		{
			...
			m_SceneGraph.ForEach(
				[this](SceneGraphElementIndex elementIndex, std::span<const glm::mat4> transformMatrices) {
					m_MatricesUniformBuffer.Bind();
					const auto& mesh = m_Meshes[elementIndex.meshId];
					mesh.Bind(transformMatrices);
...
#pragma endregion Osef
	...
	std::expected<std::vector<usize>, std::string> stw::Renderer::LoadModel(
		const std::filesystem::path& path, bool flipUVs)
	{
		Assimp::Importer importer;
		u32 assimpImportFlags = aiProcess_CalcTangentSpace | aiProcess_GenSmoothNormals
		...
		spdlog::info(
			"Converted model {} in {:0.0f} ms", pathString, timer.RestartAndGetElapsedTime().GetInMilliseconds());
	...
	stw::Renderer::Renderer(const gsl::not_null<Camera*> camera) : m_Camera(camera) {}
	...
	stw::PointLight::PointLight(const glm::vec3 position, const glm::vec3 color) : position(position), color(color)
	{
