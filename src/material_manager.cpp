	...
	const aiScene* assimpScene, const std::filesystem::path& workingDirectory, TextureManager& textureManager)
{
	...
	for (const aiMaterial* material : assimpMaterials)
	{
		...
		if ((diffuseCount > 0 || baseColorCount > 0) > 0 && normalCount > 0 && roughnessCount > 0
			&& (ambientCount > 0 || ambientOcclusionCount > 0) && metallicCount > 0)
		{
	...
	const aiMaterial* material, const std::filesystem::path& workingDirectory, stw::TextureManager& textureManager)
{
	...
	const std::size_t baseColorIndex =
		textureManager.LoadTextureFromPath(baseColorPath, TextureType::BaseColor).value();
	...
	const std::size_t roughnessIndex =
		textureManager.LoadTextureFromPath(roughnessPath, TextureType::Roughness).value();
	...
	const std::size_t ambientOcclusionIndex =
		textureManager.LoadTextureFromPath(ambientOcclusionPath, TextureType::AmbientOcclusion).value();
	...
	const std::size_t metallicIndex = textureManager.LoadTextureFromPath(metallicPath, TextureType::Metallic).value();
