...
class CubeScene final : public Scene
{
  public:
    ...
    static constexpr std::array CubePositions = {glm::vec3(0.0f, 0.0f, 0.0f),    glm::vec3(2.0f, 5.0f, -15.0f),
                                                 glm::vec3(-1.5f, -2.2f, -2.5f), glm::vec3(-3.8f, -2.0f, -12.3f),
                                                 glm::vec3(2.4f, -0.4f, -3.5f),  glm::vec3(-1.7f, 3.0f, -7.5f),
                                                 glm::vec3(1.3f, -2.0f, -2.5f),  glm::vec3(1.5f, 2.0f, -2.5f),
                                                 glm::vec3(1.5f, 0.2f, -1.5f),   glm::vec3(-1.3f, 1.0f, -1.5f)};

    CubeScene(const std::int32_t windowWidth, const std::int32_t windowHeight)
    ...
    void Update(const float deltaTime) override
    {
        ...
        for (std::uint32_t i = 0; i < 10; i++)
        {
            ...
            const float angle = 20.0f * static_cast<float>(i + 1) * m_Time;
    ...
    void OnEvent(const SDL_Event &event) override
    {
        ...
        case SDL_MOUSEMOTION: {
            const auto xOffset = static_cast<float>(event.motion.xrel);
            const auto yOffset = static_cast<float>(-event.motion.yrel);
            m_Camera.ProcessMouseMovement(xOffset, yOffset);
            break;
        }
        case SDL_MOUSEWHEEL: {
            const float yOffset = event.wheel.preciseY;
